...
void QueueToArray(Queue Q, DataType *seq)
{
    ...
    StackToArray(*Q.stack_out, temp);
    for (i = 0; i < StackLength(*Q.stack_out); i++)
    {
        seq[idx++] = temp[i];
    }
    StackToArray(*Q.stack_in, temp);
    for (i = StackLength(*Q.stack_in) - 1; i >= 0; i--)
    {
        seq[idx++] = temp[i];
    }
}
